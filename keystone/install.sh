...
sudo crudini --set /etc/keystone/keystone.conf DEFAULT verbose $OS_VERBOSE
sudo crudini --set /etc/keystone/keystone.conf DEFAULT log_dir /var/log/keystone
sudo crudini --set /etc/keystone/keystone.conf DEFAULT admin_token $OS_SERVICE_TOKEN
...
sudo crudini --set /etc/keystone/keystone.conf memcache servers $OS_IP:11211
